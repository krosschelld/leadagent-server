...
async function handleNewLead(rawLead, client) {
  ...
  console.log(`[BleedLeads] New lead for client "${client.name}" from ${rawLead.source}: ${rawLead.name}`);
  ...
  try {
    ...
    console.log(`[BleedLeads] Lead ${lead.id} processed ✓ | SMS:${notified.sms} Email:${notified.email} CRM:${crmResult.success ? crmResult.leadId : "failed — " + crmResult.error}`);
  } catch (err) {
